...
public final class NormalizerFactory {
    ...
    public static Normalizer<byte[]> jsonObject(List<EnrichmentRule> rules, String rootPath) {
        return new Normalizer<>(Pipeline.<byte[]>start()
                ...
                .add(new ObservedStep<>("extract", new ExtractStep<>(PayloadExtractorFactory.withRules(rules))))
                .add(new ObservedStep<>("context-locate", new ContextLocateStep<>(PayloadLocatorFactory.fromRootPath(rootPath))))
                .add(new ObservedStep<>("split-enrich", new SplitEnrichStep<>(PayloadSplitterFactory.single(), RecordEnricherFactory.json())))
                .add(new ObservedStep<>("serialize", new SerializeStep<>(RecordSerializerFactory.jsonNode()))));
    }

    public static Normalizer<ResultSet> resultSet() {
        return new Normalizer<>(Pipeline.<ResultSet>start()
                .add(new ObservedStep<>("split", new SplitStep<>(PayloadSplitterFactory.resultSet())))
